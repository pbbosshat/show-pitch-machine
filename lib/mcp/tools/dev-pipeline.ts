// MCP tool handlers for the MYE development pipeline and project timelines.
// These tools surface the data imported from the MYE Google Sheet — projects grouped
// by sheet_source, plus a full email/story-scout timeline for any individual project.
//
// Data model notes:
//   • ip_catalog rows with sheet_row_key IS NOT NULL came from the spreadsheet import.
//   • sheet_source values map to sheet tabs: priorities, bc-mye, full-dev, backburner,
...
// Raw row returned by the pipeline query (ip_catalog + aggregate email stats)
...
// Sizzle reel row from sizzle_reels table
...
// Full ip_catalog row (all columns, including sheet extension columns)
...
// A single email thread linked to a project
...
// A story scout row (linked via project_banner LIKE match)
...
// Canonical display order for sheet_source groups — mirrors the spreadsheet tab order
...
// Section headers shown in the formatted output for each source group
 ...
 * Example: "• Pros vs Joes ▶ SIZZLE — Point: MY/JT | Targets: NBC, Hulu, Fox | 13 threads | Last: 2026-05-02"
 */
function formatPipelineRow(row: PipelineRow): string {
  const parts: string[] = [];

  // Title + sizzle indicator — always present
  ...
  // Point person
  if (row.sheet_point_person) {
    parts.push(`Point: ${row.sheet_point_person}`);
  }

  // Status (sheet_status if present, else derived status)
  const displayStatus = row.sheet_status || row.status;
  if (displayStatus) {
    parts.push(`Status: ${displayStatus}`);
  }

  // Target networks — cap at 3 to keep lines readable
  ...
  // Pitched-to networks
  if (row.sheet_pitched_to) {
    parts.push(`Pitched: ${row.sheet_pitched_to}`);
  }

  // Email activity
  const threadCount = row.email_thread_count ?? 0;
  parts.push(`${threadCount} email thread${threadCount === 1 ? '' : 's'}`);
  const lastActivity = row.last_email_date ?? 'no email activity found';
  parts.push(`Last: ${lastActivity}`);

  // Join with em-dash separator for the title, then pipes for the rest
...
/**
 * Returns all ip_catalog rows that came from the spreadsheet import, grouped by
...
export function getDevelopmentPipeline(filters: {
  ...
  // Render section by section in SOURCE_ORDER, then any unknown sources at the end
  const orderedKeys = [
...
/**
 * Returns a full detail view for a single project: ip_catalog record, complete
 * email thread timeline sorted newest-first, and any story scout connections.
 *
...
export function getProjectTimeline(projectName: string): string {
  ...
  // Sizzle assets section — only show if sizzles exist
  if (sizzles.length > 0) {
    lines.push('');
  ...
  if (threads.length === 0) {
    ...
    for (const t of threads) {
      // Direction indicator: received = inbound from buyer; sent = outbound MYE
      ...
      // Show the first external participant (non-MYE address) if we can parse it
      let participantStr = '';
      if (t.participants) {
        try {
          const parsed: string[] = JSON.parse(t.participants);
          // Exclude common MYE domain addresses so we show the buyer contact
          const external = parsed.find(
            (p) => !p.includes('@myentertainment') && !p.includes('@gototeam')
          );
          if (external) participantStr = ` — ${external}`;
        } catch {
          // participants is not valid JSON — show raw truncated value
...
/**
 * Returns a complete sizzle reel inventory: all sizzle_reels rows with email thread
 * counts and last activity dates. Grouped into two categories:
 *   1. WITH VIDEO LINK — sizzles with vimeo_url populated
...
export function getSizzleInventory(): string {
  ...
  // Partition into with-video and without-video groups
  const withVideo = sizzles.filter((s) => s.vimeo_url);
  const withoutVideo = sizzles.filter((s) => !s.vimeo_url);

  const lines: string[] = [];
  lines.push(
    `SIZZLE REEL INVENTORY — ${sizzles.length} total (${withVideo.length} with video links)`
  );
  lines.push('━'.repeat(50));
  lines.push('');

  // Section 1: WITH VIDEO LINK
  ...
  // Section 2: CONFIRMED EXISTS — NO LINK YET
