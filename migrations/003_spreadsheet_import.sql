-- migrations/003_spreadsheet_import.sql
...
-- This migration is designed to run exactly once after 001 and 002.
-- The import script (scripts/import-mye-sheet.ts) uses sheet_row_key as its upsert key.

-- ── Extend ip_catalog ──────────────────────────────────────────────────────────

-- Which sheet tab this row came from (e.g. 'Priorities', 'Passes', 'Brainstorms')
ALTER TABLE ip_catalog ADD COLUMN sheet_source TEXT;

-- Raw Status column value from the spreadsheet (e.g. 'In Development', 'Passed')
ALTER TABLE ip_catalog ADD COLUMN sheet_status TEXT;

-- Point person listed in the spreadsheet (internal MYE exec responsible for the project)
ALTER TABLE ip_catalog ADD COLUMN sheet_point_person TEXT;

-- Comma-separated list of target networks from the sheet (e.g. 'A&E, Discovery, Netflix')
ALTER TABLE ip_catalog ADD COLUMN sheet_target_nets TEXT;

-- Who the project was pitched to (buyer name or network, from "Pitching/Pitched" column)
ALTER TABLE ip_catalog ADD COLUMN sheet_pitched_to TEXT;

-- Passed buyers or pass notes from the "Passed" column
ALTER TABLE ip_catalog ADD COLUMN sheet_passed TEXT;

-- Next steps timing / deadline text from the sheet
ALTER TABLE ip_catalog ADD COLUMN sheet_next_steps TEXT;

-- Attachments field from the sheet (sizzle/deck availability notes)
ALTER TABLE ip_catalog ADD COLUMN sheet_attachments TEXT;

-- The full raw combined status string before normalization
ALTER TABLE ip_catalog ADD COLUMN sheet_raw_status TEXT;

-- Best date string extracted from next_steps or status text (ISO-ish, e.g. '2024-03-15')
ALTER TABLE ip_catalog ADD COLUMN extracted_date TEXT;

-- Confidence level for the extracted date: 'extracted' | 'year-only' | 'none'
ALTER TABLE ip_catalog ADD COLUMN date_confidence TEXT DEFAULT 'none';

-- Rank from the Brainstorms sheet (lower = higher priority)
ALTER TABLE ip_catalog ADD COLUMN brainstorm_rank INTEGER;
...
ALTER TABLE ip_catalog ADD COLUMN sheet_row_key TEXT;
...
ALTER TABLE buyer_contacts ADD COLUMN region TEXT DEFAULT 'us';

-- 1 if contact came from "Former A&E Execs" sheet — useful for targeting/filtering
ALTER TABLE buyer_contacts ADD COLUMN is_former INTEGER DEFAULT 0;
...
-- Populated by scripts/cross-ref-emails.ts which searches Gmail for threads
-- that mention each project by title and links them here.
