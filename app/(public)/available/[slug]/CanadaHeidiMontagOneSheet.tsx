...
// Identical palette to HeidiMontagRookiePiOneSheet — same show, Canadian cut
const PINK   = '#E0257A';
const BLACK  = '#0C080F';
const PANEL  = '#1A1025';
const WHITE  = '#F5F0F8';
...
// Maple-leaf red for the Canadian market accent badge
const CANADA_RED = '#D52B1E';

// Pink/black diagonal stripe divider — same as HeidiMontagRookiePiOneSheet
...
function PasswordGate({ onUnlock }: { onUnlock: () => void }) {
  ...
  async function submit(e: React.FormEvent) {
    ...
    const res = await fetch('/api/marketing/available/verify-password', {
      ...
      body: JSON.stringify({ slug: 'canada-heidi-montag', password: pw }),
    });
    if (res.ok) {
      onUnlock();
    } else {
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <p style={{ color: MUTED, fontSize: 14, marginBottom: 28 }}>Canadian Broadcaster Presentation · Enter password to view</p>
...
export default function CanadaHeidiMontagOneSheet({ title }: { title: SafeTitle }) {
  const [unlocked, setUnlocked] = useState(false);

  // Enforce password gate for protected titles
  if (title.has_password && !unlocked) {
    return <PasswordGate onUnlock={() => setUnlocked(true)} />;
  }

  // No sizzle reel for this cut — placeholder renders instead
  ...
  return (
    <div style={{ background: BLACK, color: WHITE, fontFamily: "'Roboto', sans-serif", lineHeight: 1.6 }}>
      ...
      <section style={{ position: 'relative', height: '100vh', overflow: 'hidden', minHeight: 600 }}>
        <img
          src="/available-thumbs/chm.png"
          alt="Heidi Montag: Rookie PI — Canadian Broadcaster Presentation"
          style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover', objectPosition: 'center top' }}
        />
        {/* Gradient overlay — fade hero into page background */}
        <div style={{
        ...
        <div style={{
          ...
          {/* Format badge + Canadian Market badge side-by-side */}
          <div style={{ marginBottom: 16, display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'center' }}>
            ...
            {/* Canadian Market badge — distinct red to signal this is the CA cut */}
            <span style={{
              display: 'inline-block', padding: '5px 14px',
              background: `${CANADA_RED}22`, border: `1px solid ${CANADA_RED}88`,
              borderRadius: 3, color: '#FF5A4F', fontSize: 11,
              fontFamily: "'Roboto Condensed', sans-serif",
              fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.18em',
            }}>🍁 Canadian Market</span>
          </div>
          <h1 style={{
            fontFamily: "'Roboto Condensed', sans-serif",
            fontWeight: 900, fontSize: 'clamp(48px, 7vw, 96px)',
            textTransform: 'uppercase', letterSpacing: '-0.01em',
            lineHeight: 0.9, margin: '0 0 16px',
            textShadow: '0 4px 24px rgba(0,0,0,0.7)',
          }}>
            Heidi Montag:<br />
            <span style={{ color: PINK }}>Rookie PI</span>
          </h1>
          {/* Canadian broadcaster subtitle — differentiates this deck from the standard one */}
          <p style={{
            fontSize: 'clamp(13px, 1.6vw, 17px)', fontWeight: 700,
            color: `${WHITE}99`, maxWidth: 560, margin: '0 0 8px',
            fontFamily: "'Roboto Condensed', sans-serif",
            textTransform: 'uppercase', letterSpacing: '0.12em',
          }}>
            Canadian Broadcaster Presentation
          </p>
          <p style={{
          ...
          <p style={{
            ...
            MY Entertainment · Presented for Canadian Broadcasters
          </p>
        </div>
      </section>
      ...
      {/* ── CANADIAN BROADCASTER NOTE ── */}
      <section style={{ background: PANEL, padding: '48px 48px' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          <div style={{
            border: `1px solid ${CANADA_RED}55`, borderRadius: 6,
            padding: '28px 32px', background: `${CANADA_RED}0A`,
            ...
            <div>
              <p style={{
                fontFamily: "'Roboto Condensed', sans-serif", fontWeight: 900,
                fontSize: 13, color: '#FF5A4F', letterSpacing: '0.2em',
                textTransform: 'uppercase', marginBottom: 8,
              }}>Presented for Canadian Broadcasters</p>
              <p style={{ color: `${WHITE}BB`, fontSize: 15, lineHeight: 1.75, margin: 0 }}>
                This presentation has been prepared specifically for Canadian broadcasters and network partners.
                The format&apos;s investigative structure and celebrity-led access make it an ideal acquisition
                for Canadian premium factual slots.
              </p>
            </div>
          </div>
        </div>
      ...
      <section style={{ background: PANEL, padding: '80px 48px' }}>
        <div style={{ maxWidth: 700, margin: '0 auto', textAlign: 'center' }}>
          ...
          <p style={{
            ...
            Heidi Montag: Rookie PI · Premium Factual Series · Canadian Broadcasters
          </p>
          <a
            href="mailto:info@myentertainment.tv?subject=Heidi Montag: Rookie PI — Canadian Broadcaster Inquiry"
            style={{
