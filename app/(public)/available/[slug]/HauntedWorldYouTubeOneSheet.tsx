  ...
  async function handlePasswordSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch('/api/marketing/available/verify-password', {
        ...
        body: JSON.stringify({ slug: 'haunted-world-youtube', password: pw }),
      });
      if (res.ok) { onUnlock(); }
      else {
  ...
  async function handleRequestSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch('/api/contact', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ ...reqForm, slug: 'haunted-world-youtube' }),
      });
...
export default function HauntedWorldYouTubeOneSheet({ title }: { title: SafeTitle }) {
      ...
      <section style={{ position: 'relative', minHeight: '100vh', display: 'flex', flexDirection: 'column', justifyContent: 'flex-end', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'relative', zIndex: 2, padding: '0 40px 80px', maxWidth: 900 }}>
          <p style={{ fontFamily: "'Roboto Condensed', sans-serif", fontWeight: 700, fontSize: 12, letterSpacing: '0.35em', textTransform: 'uppercase', color: GOLD, margin: '0 0 20px' }}>
            A Paranormal History Documentary Series for Digital Audiences
          </p>
          <h1 style={{ fontFamily: "'Roboto Condensed', sans-serif", fontWeight: 900, fontSize: 'clamp(64px, 10vw, 120px)', textTransform: 'uppercase', lineHeight: 0.88, margin: '0 0 12px', letterSpacing: '-2px', color: MIST }}>
            Haunted<br />
            <span style={{ color: BLUE }}>World</span>
          </h1>
          <p style={{ fontFamily: "'Roboto Condensed', sans-serif", fontWeight: 700, fontSize: 14, letterSpacing: '0.2em', textTransform: 'uppercase', color: `${BLUE}99`, margin: '0 0 20px' }}>
            Digital Series
          </p>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
            <span style={{ fontFamily: "'Roboto Condensed', sans-serif", fontSize: 12, fontWeight: 700, letterSpacing: '0.2em', textTransform: 'uppercase', color: '#7a8fa0', border: '1px solid #2a3a4e', padding: '7px 16px', borderRadius: 3 }}>
              Digital Documentary Series · 8 × 30 min
            </span>
            <span style={{ fontFamily: "'Roboto Condensed', sans-serif", fontSize: 12, fontWeight: 700, letterSpacing: '0.2em', textTransform: 'uppercase', color: '#7a8fa0', border: '1px solid #2a3a4e', padding: '7px 16px', borderRadius: 3 }}>
              Global Locations
            </span>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: 70, alignItems: 'center' }}>
            <div>
              ...
              <p style={{ fontFamily: "'Roboto Condensed', sans-serif", fontWeight: 700, fontSize: 'clamp(30px, 3.5vw, 46px)', lineHeight: 1.05, color: MIST, textTransform: 'uppercase', margin: 0 }}>
                ...
                <span style={{ color: BLUE }}>about ghosts.</span>
              </p>
            </div>
            <div>
              <p style={{ fontSize: 18, lineHeight: 1.85, color: '#b8c8d8', margin: '0 0 24px' }}>
                Haunted World is a paranormal history documentary series that goes beyond the scare — exploring the true and terrifying stories behind history&apos;s most haunted places worldwide.
              </p>
              <p style={{ fontSize: 17, lineHeight: 1.8, color: '#8a9aaa', margin: '0 0 24px' }}>
                Each location carries a real history of tragedy, violence, obsession, or ritual that created the haunting in the first place. <em style={{ color: GOLD }}>Every haunting begins with a story.</em> This series tells the ones history forgot.
              </p>
              <div style={{ marginTop: 32, padding: '22px 28px', borderLeft: `4px solid ${BLUE}`, background: `rgba(107,163,190,0.06)` }}>
                <p style={{ fontFamily: "'Roboto Condensed', sans-serif", fontWeight: 700, fontSize: 20, color: BLUE, margin: 0, fontStyle: 'italic', lineHeight: 1.5 }}>
                  &ldquo;This is not a ghost show. This is a history show that happens to be about ghosts.&rdquo;
                </p>
              </div>
            </div>
          </div>
      ...
      <section style={{ padding: '90px 40px', background: BLACK }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: 64 }}>
            <SectionLabel>Digital Format</SectionLabel>
            <h2 style={{ fontFamily: "'Roboto Condensed', sans-serif", fontWeight: 900, fontSize: 'clamp(32px, 4vw, 52px)', color: MIST, textTransform: 'uppercase', margin: '0 0 24px', letterSpacing: '-0.5px' }}>
              Built for Digital Audiences
            </h2>
            <p style={{ fontSize: 18, lineHeight: 1.8, color: '#8a9aaa', maxWidth: 780, margin: '0 auto' }}>
              Produced in a streamlined format optimized for digital audiences, each episode is a focused, immersive 30-minute deep-dive into a single location — its history, its haunting, and the people who experienced it firsthand.
            </p>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 32 }}>
            {[
              { icon: '📍', label: 'Single Location Focus', desc: 'Each episode zooms in on one location — giving digital viewers a complete, immersive experience in a single session.' },
              { icon: '⏱', label: '30-Minute Deep Dive', desc: 'Optimized for the streaming and digital viewing window: long enough to immerse, tight enough to retain.' },
              { icon: '🌍', label: 'Global Locations', desc: 'Each episode travels to a different haunted location worldwide — maximum variety for platform algorithms and international audiences.' },
            ].map(item => (
        ...
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 32, textAlign: 'center' }}>
            {[
              ...
              { value: '30-Minute', label: 'Format' },
              { value: 'Global', label: 'Locations' },
              { value: 'Paranormal History', label: 'Genre' },
            ].map(s => (
              <div key={s.label}>
      ...
      <section style={{ padding: '90px 40px', background: DARK }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: 70 }}>
            ...
            <div>
              <SectionLabel>Network &amp; Platform Partners</SectionLabel>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: 40 }}>
                {['Max', 'Discovery+', 'A&E', 'YouTube', 'VICE', 'PBS', 'National Geographic', 'BBC', 'Lifetime', 'Travel Channel', 'ID: Investigation Discovery', 'Oxygen'].map(n => (
      ...
      <section style={{ padding: '100px 40px', background: BLACK, textAlign: 'center' }}>
        <div style={{ maxWidth: 700, margin: '0 auto' }}>
          ...
          <p style={{ fontFamily: "'Roboto Condensed', sans-serif", fontSize: 13, fontWeight: 700, letterSpacing: '0.2em', textTransform: 'uppercase', color: '#555', margin: '0 0 48px' }}>
            MY Entertainment · Haunted World (Digital Series)
          </p>
          <a
            href="mailto:info@myentertainment.tv?subject=Haunted World Digital Series — Inquiry"
