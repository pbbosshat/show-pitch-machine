  ...
  async function handlePasswordSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch('/api/marketing/available/verify-password', {
        ...
        body: JSON.stringify({ slug: 'home-game-athletes-first', password: pw }),
      });
      if (res.ok) { onUnlock(); }
      else {
  ...
  async function handleRequestSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch('/api/contact', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ ...reqForm, slug: 'home-game-athletes-first' }),
      });
...
export default function HomeGameAthletesFirstOneSheet({ title }: { title: SafeTitle }) {
    ...
    <div style={{ background: BLACK, minHeight: '100vh', color: CREAM, fontFamily: "'Roboto', sans-serif" }}>
      ...
      <section style={{ position: 'relative', minHeight: '100vh', display: 'flex', flexDirection: 'column', justifyContent: 'flex-end', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'relative', zIndex: 2, padding: '0 40px 80px', maxWidth: 900 }}>
          ...
          {/* Athletes First presentation badge */}
          <div style={{ marginBottom: 20 }}>
            <span style={{
              ...
              fontFamily: "'Roboto Condensed', sans-serif",
              fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.18em',
            }}>Presented to Athletes First</span>
          </div>
      ...
      {/* ── 3. ATHLETES FIRST PARTNERSHIP ────────────────────────────── */}
      {/* Partnership section specific to Athletes First presentation */}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 64, alignItems: 'center' }}>
            <div>
              ...
              <h2 style={{ fontFamily: "'Roboto Condensed', sans-serif", fontWeight: 900, fontSize: 'clamp(28px, 3vw, 40px)', color: CREAM, textTransform: 'uppercase', lineHeight: 1.05, margin: '0 0 24px' }}>
                Life Beyond<br /><span style={{ color: GOLD }}>The Game.</span>
              </h2>
            </div>
            <div>
              <p style={{ fontSize: 16, lineHeight: 1.8, color: `${CREAM}CC`, margin: 0 }}>
                Athletes First represents professional athletes who understand the importance of building life beyond the game. Home Game gives those athletes an authentic, audience-first platform to showcase their lives, their families, and the homes that ground them.
              </p>
            </div>
          </div>
        </div>
      ...
      <section style={{ padding: '100px 40px', background: DARK, textAlign: 'center' }}>
        <div style={{ maxWidth: 700, margin: '0 auto' }}>
          ...
          <p style={{ fontFamily: "'Roboto Condensed', sans-serif", fontSize: 13, fontWeight: 700, letterSpacing: '0.2em', textTransform: 'uppercase', color: MUTED, margin: '0 0 48px' }}>
            MY Entertainment · Home Game · Athletes First
          </p>
          <a
            href="mailto:info@myentertainment.tv?subject=Home Game — Athletes First Inquiry"
