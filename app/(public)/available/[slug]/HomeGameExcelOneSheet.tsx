  ...
  async function handlePasswordSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch('/api/marketing/available/verify-password', {
        ...
        body: JSON.stringify({ slug: 'home-game-excel', password: pw }),
      });
      if (res.ok) { onUnlock(); }
      else {
  ...
  async function handleRequestSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch('/api/contact', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ ...reqForm, slug: 'home-game-excel' }),
      });
    ...
    <div style={{ position: 'fixed', inset: 0, zIndex: 100, background: 'rgba(0,0,0,0.95)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
      <div style={{ maxWidth: 440, width: '100%', background: DARK, border: `1px solid ${GREEN}44`, borderRadius: 12, padding: 40 }}>
        ...
        <p style={{ fontSize: 12, color: GOLD, textAlign: 'center', letterSpacing: '0.15em', textTransform: 'uppercase', margin: '0 0 28px' }}>
          Excel Sports Management Presentation
        </p>
...
// Why it works — 3 value cards (identical to base HomeGameOneSheet)
const WHY_IT_WORKS = [
  {
    icon: '🏆',
...
export default function HomeGameExcelOneSheet({ title }: { title: SafeTitle }) {
    ...
    <div style={{ background: BLACK, minHeight: '100vh', color: CREAM, fontFamily: "'Roboto', sans-serif" }}>
      ...
      <section style={{ position: 'relative', minHeight: '100vh', display: 'flex', flexDirection: 'column', justifyContent: 'flex-end', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'relative', zIndex: 2, padding: '0 40px 80px', maxWidth: 900 }}>
          ...
          {/* Excel Sports Management presentation badge */}
          <div style={{ marginBottom: 20 }}>
            <span style={{
              ...
              fontFamily: "'Roboto Condensed', sans-serif",
              fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.18em',
            }}>Presented to Excel Sports Management</span>
          </div>
      ...
      {/* ── 3. EXCEL PARTNERSHIP ─────────────────────────────────────── */}
      {/* Partnership section specific to Excel Sports Management presentation */}
      <section style={{ padding: '80px 40px', background: BLACK }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 64, alignItems: 'center' }}>
            <div>
              <SectionLabel color={GOLD}>Partnership</SectionLabel>
              <h2 style={{ fontFamily: "'Roboto Condensed', sans-serif", fontWeight: 900, fontSize: 'clamp(28px, 3vw, 40px)', color: CREAM, textTransform: 'uppercase', lineHeight: 1.05, margin: '0 0 24px' }}>
                Built for<br /><span style={{ color: GOLD }}>Elite Athletes.</span>
              </h2>
            </div>
            <div>
              <p style={{ fontSize: 16, lineHeight: 1.8, color: `${CREAM}CC`, margin: 0 }}>
                Excel Sports Management represents some of the most elite athletes in professional sports. Home Game gives those athletes an exclusive platform to share their lives beyond the field — connecting them with audiences in an authentic, heartwarming format that builds lasting personal brands.
              </p>
            </div>
          </div>
        </div>
      ...
      {/* ── 10. CTA ───────────────────────────────────────────────────── */}
      <section style={{ padding: '100px 40px', background: DARK, textAlign: 'center' }}>
        <div style={{ maxWidth: 700, margin: '0 auto' }}>
          ...
          <p style={{ fontFamily: "'Roboto Condensed', sans-serif", fontSize: 13, fontWeight: 700, letterSpacing: '0.2em', textTransform: 'uppercase', color: MUTED, margin: '0 0 48px' }}>
            MY Entertainment · Home Game · Excel Sports Management
          </p>
          <a
            href="mailto:info@myentertainment.tv?subject=Home Game — Excel Sports Management Inquiry"
