/**
 * PATCH /api/prodcos/[id]/verify
 * Called by: prodco detail page verify button
 * Auth: none
 * Body: { verified_by?: string }   (defaults to 'manual' if omitted)
 * Response: { data: { changes: number } }
 *
 * Sets is_verified=1, verified_at=now(), verified_by on the production_companies row.
 * Returns 404 if the production company does not exist.
...
export async function PATCH(
  ...
  try {
    ...
    // Confirm the prodco exists before attempting the update
