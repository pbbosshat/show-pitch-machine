 ...
 *         shows (search_vector GIN index), IP catalog, packages, talent, deck sites.
 */
...
/**
 * safeQuery wraps an async query so that one failing entity type never kills
 * the entire search response — just returns an empty array for that type.
 */
...
export async function GET(request: NextRequest) {
  ...
  // ── Buyer contacts ──────────────────────────────────────────────────────────
  const buyerContacts = await safeQuery<{ id: string; name: string; email: string | null; title: string | null; company_name: string | null }>(
  ...
  buyerContacts.forEach(r => results.push({
    type: 'Buyer Contact',
    id: r.id,
    title: r.name,
  ...
  const buyerCompanies = await safeQuery<{ id: string; name: string; type: string | null; hq_city: string | null }>(
    `SELECT id, name, type, hq_city FROM buyer_companies WHERE name LIKE ? LIMIT 5`,
    [like]
  );
  buyerCompanies.forEach(r => results.push({
    type: 'Network / Buyer',
    id: r.id,
    title: r.name,
  ...
  const prodcos = await safeQuery<{ id: string; name: string; ownership_type: string | null; hq_city: string | null }>(
  ...
  prodcos.forEach(r => results.push({
    type: 'Prod Co',
    id: r.id,
    title: r.name,
  ...
  // ── Prodco contacts ─────────────────────────────────────────────────────────
  const prodcoContacts = await safeQuery<{ id: string; name: string; title: string | null; email: string | null; company_name: string | null }>(
  ...
  prodcoContacts.forEach(r => results.push({
  ...
  // ── Shows — Postgres tsvector full-text search via search_vector GIN index ──
  // search_vector is a generated column on shows (tsvector over title + genre +
  // network + production_company). plainto_tsquery parses plain English terms
  // without requiring FTS5 MATCH syntax.
  const ftsQuery = q.replace(/[^a-zA-Z0-9 ]/g, '').trim();
  const shows = ftsQuery.length >= 2
    ? await safeQuery<{ id: string; title: string; network: string | null; genre: string | null }>(
        `SELECT id, title, network, genre
         FROM shows
  ...
  // Fall back to LIKE if the tsvector search returned nothing (query too narrow / no match)
  ...
  // ── IP catalog ──────────────────────────────────────────────────────────────
  const ipItems = await safeQuery<{ id: string; title: string; format: string | null; genre: string | null }>(
  ...
  ipItems.forEach(r => results.push({
    type: 'IP / Project',
    id: r.id,
    title: r.title,
  ...
  // ── Packages ────────────────────────────────────────────────────────────────
  const packages = await safeQuery<{ id: string; name: string; pipeline_stage: string | null; status: string | null }>(
    `SELECT id, name, pipeline_stage, status FROM packages WHERE name LIKE ? LIMIT 5`,
    [like]
  );
  packages.forEach(r => results.push({
    type: 'Package',
    id: r.id,
    title: r.name,
  ...
  // ── Talent ──────────────────────────────────────────────────────────────────
  const talent = await safeQuery<{ id: string; name: string; primary_role: string | null; talent_tier: string | null }>(
    `SELECT id, name, primary_role, talent_tier FROM talent WHERE name LIKE ? LIMIT 5`,
    [like]
  );
  talent.forEach(r => results.push({
    type: 'Talent',
    id: r.id,
    title: r.name,
  ...
  // ── Deck sites ──────────────────────────────────────────────────────────────
  const decks = await safeQuery<{ id: string; title: string; genre: string | null; status: string | null }>(
    `SELECT id, title, genre, status FROM deck_sites WHERE title LIKE ? OR slug LIKE ? LIMIT 5`,
    [like, like]
  );
  decks.forEach(r => results.push({
    type: 'Deck',
    id: r.id,
    title: r.title,
