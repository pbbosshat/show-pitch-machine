 ...
 *
 * mode=pending — rolling 7-day uncleared stack. Returns all leads where status is
 * still actionable (not yet sent/skipped/failed), from the last 7 days, ordered by
...
export async function GET(request: Request) {
  try {
    ...
    if (mode === 'pending') {
      // Rolling uncleared stack — all leads from the past 7 days that are still
      ...
      rows = await query<JoinedLead>(
        `SELECT cl.*, ta.headline AS article_headline, ta.url AS article_url
          ...
          WHERE cl.status IN ('new', 'enriching', 'ready', 'queued')
            AND cl.lead_date >= to_char(CURRENT_DATE - INTERVAL '7 days', 'YYYY-MM-DD')
